...
# from prediction import predict_top_roles_domains, get_gap_skills

# # ------------------ Streamlit Config ------------------ #
# st.set_page_config(page_title="🔍 SkillFit: Role Matcher & Upskill Analyzer", layout="wide")

# st.markdown(
#     "<h1 style='text-align: center; color: #1A5276;'>🚀 SkillFit: Smart Resume Analyzer & Role Recommender</h1>",
...
#     with st.sidebar.expander("🖼️ Preview Uploaded Resume"):
...
#     # Extract Skills
#     skill_json_path = "data/skills.json"
#     extracted_skills = extract_skills_from_pdf(tmp_pdf_path, skill_json_path)
...
#         st.write("Swipe through the top 3 role matches to explore personalized improvement areas:")

#         for i, (role, domain, score) in enumerate(top_5[:3], start=1):
#             with st.expander(f"🔎 Rank {i}: {role} | {domain}"):
...
#                     st.write(", ".join(sorted([s.title() for s in user])) if user else "_None_")

#                 with col2:
#                     st.markdown(f"📌 **Required Skills ({len(required)}):**")
#                     st.write(", ".join(sorted([s.title() for s in required])) if required else "_Not Specified_")

#                 with col3:
#                     st.markdown(f"⚠️ **Gap Skills ({len(gap)}):**")
#                     if gap:
#                         for skill in sorted(gap):
#                             skill_title = skill.title()
#                             search_url = f"https://www.google.com/search?q=learn+{skill.replace(' ', '+')}+online"
#                             st.markdown(f"- 🔺 _{skill_title}_ → [Upskill Link]({search_url})")
#                     else:
    ...
    if extracted_skills:
        ...
        if generate_pdf_btn:
            ...
            if os.path.exists(pdf_report_path) and os.path.getsize(pdf_report_path) > 0:
                ...
                st.sidebar.error("⚠️ PDF report generation failed.")
